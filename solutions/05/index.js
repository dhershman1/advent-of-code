...
const dataSet = await getData(false)
  ...
  for (let i = 0; i < transforms.length; i++) {
    ...
    for (let j = 0; j < seeds.length; j++) {
      ...
      if (intersections.length) {
        ...
        if (intersections[0][0] > bot) { nSeeds.push([bot, intersections[0][0] - 1]) }
        if (intersections[intersections.length - 1][1] < top) { nSeeds.push([intersections[intersections.length - 1][1] + 1, top]) }
        for (let k = 0; k < intersections.length - 2; k++) {
          if (intersections[k + 1][0] - intersections[k][1] > 1) { nSeeds.push([intersections[k][1] + 1, intersections[k + 1][0] - 1]) }
        }
